 ...
 * AUTHOR       :  (  )
 * FILENAME     : QSkinWidget.cpp
 * LICENSE      :
 * COMMENTARY   :
 ********************************************************************************************************/
#include <QtGui>
#include "QSkinWidget.h"
QSkinWidgetResizeHandler::QSkinWidgetResizeHandler(QSkinWidget *parent, QWidget *cw) : QWidgetResizeHandler(parent, cw)
{
    skinWidget = parent;
}
bool QSkinWidgetResizeHandler::eventFilter(QObject *o, QEvent *e)
{
...
QSkinWidget::QSkinWidget( QWidget* wgtParent)
    : QWidget(wgtParent)

{
    ...
    //Init the childForm
    setWindowTitle("QSkinWidget");
    setMinimumSize (100, 32);
    //get our frame and close-button
    alpha = 255;
    widgetMask = QPixmap(width(), height());
    //let the window repaint every 100ms (is needed for the close-button)
    ...
    resizeHandler =  new QSkinWidgetResizeHandler(this);

    resizeFrame = true;

}
void QSkinWidget::manageRegions()
{
...
void QSkinWidget::getSelectedFrame(QPoint p)
{
...
void QSkinWidget::closeEvent(QCloseEvent *e)
{
    emit aboutToClose();
}
void QSkinWidget::resizeEvent(QResizeEvent *e)
{
    manageRegions();
    resizeFrame = true;
}
void QSkinWidget::mousePressEvent(QMouseEvent *e)
 {
    // winwow Mover
...
void QSkinWidget::mouseReleaseEvent(QMouseEvent *mouseEvent)
{

}
void QSkinWidget::mouseMoveEvent(QMouseEvent *e)
{
...
void QSkinWidget::setSkinPath(const QString & skinpath)
{
    skinPath = skinpath;
    update();

}
QString QSkinWidget::getSkinPath()
{
    return skinPath;
}

void QSkinWidget::updateStyle()
{
...
void QSkinWidget::paintEvent(QPaintEvent *event)
{
    if(resizeFrame)
        updateStyle();
    QPainter p(this);
