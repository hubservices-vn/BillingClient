...
const QString Wallpaper_Folder_Name = "Wallpapers";
const QString Icon_Folder_Name = "Icon";
...
bool PathManager::ensureFolderExists(QString &folderPath)
{
    return _ensureFolderExists(folderPath);
}
...
QString PathManager::getAppFolder()
{
    ...
    s_appFolder = SysInfoHelper::allUserAppDataFolder() + "/" + APP_DATA_FOLDER_NAME;
...
QString PathManager::getDBFolder()
{
    ...
    s_dbFolder = getAppFolder() + "\\" + DB_Folder_Name;
    _ensureFolderExists(s_dbFolder);

    return s_dbFolder;
}

QString PathManager::getWallpaperFolder()
{
    static QString s_wpFolder;
    if (s_wpFolder.isEmpty() == false)
    {
        return s_wpFolder;
    }

    s_wpFolder = getAppFolder() + "\\" + Wallpaper_Folder_Name;
    _ensureFolderExists(s_wpFolder);

    return s_wpFolder;
}


QString PathManager::getIconFolder()
{
    static QString s_dbFolder;
    if (s_dbFolder.isEmpty() == false)
    {
    ...
    s_dbFolder = QApplication::applicationDirPath() + "\\" + Icon_Folder_Name;
    _ensureFolderExists(s_dbFolder);
