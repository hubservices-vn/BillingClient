...
, m_bDebug(FALSE)
, m_sshStatusHandle(0)
, m_dwControlsAccepted(SERVICE_ACCEPT_STOP)	//SERVICE_ACCEPT_STOP
, m_pUserSID(0)
, m_fConsoleReady(FALSE)
// parameters to the "CreateService()" function:
, m_dwDesiredAccess(SERVICE_ALL_ACCESS)	//SERVICE_ALL_ACCESS
...
BOOL CNTService :: RegisterService( int argc, _TCHAR ** argv ) {
        ...
        if( schSCManager ) {
            ...
            if( schService ) {
...

                {
                    ...
                    sc_action[0].Delay = 1000;
                    sc_action[0].Type = SC_ACTION_RESTART;
                    sc_action[1].Delay = 1000;
                    sc_action[1].Type = SC_ACTION_RESTART;
                    sc_action[2].Delay = 1000;
                    sc_action[2].Type = SC_ACTION_RESTART;

                    SERVICE_FAILURE_ACTIONS sf_action;
                    sf_action.dwResetPeriod = INFINITE;
                    sf_action.lpRebootMsg = NULL;
                    sf_action.lpCommand = NULL;
                    sf_action.cActions = 3;
                    sf_action.lpsaActions = sc_action;

                    if(!ChangeServiceConfig2(schService, SERVICE_CONFIG_FAILURE_ACTIONS, &sf_action))                  
                    {
...
BOOL CNTService :: DebugService(int argc, _TCHAR ** argv, BOOL faceless) {
    int Argc;
    LPTSTR *lpszArgv;
    ...
    if( !faceless ) {	//!! TCW MOD - no faceless, so give it a face.
        ...
        BOOL b=SetConsoleCtrlHandler(ControlHandler, TRUE);
    }
    ...
    if( lRet == ERROR_SUCCESS ) {
        ...
        if( lRet == ERROR_SUCCESS ) {
            ...
            if( lRet == ERROR_SUCCESS ) {
                ...
                DWORD i = 0;
                for(; i < dwSize; i += _tcslen(p)+1, p += _tcslen(p)+1 ) {
...
void CNTService::QueryServicePath(TCHAR szPath[])
{
    BOOL bRet = FALSE;
    ...
    if( schSCManager ) {
        ...
        char buf[4096];
        LPQUERY_SERVICE_CONFIG lpServiceConfig=(LPQUERY_SERVICE_CONFIG)buf;
        DWORD dwNeed;
        QueryServiceConfig(schService,lpServiceConfig,sizeof(buf),&dwNeed);
        _tcscpy(szPath,lpServiceConfig->lpBinaryPathName);
        CloseServiceHandle(schService);
        CloseServiceHandle(schSCManager);
    }
}
