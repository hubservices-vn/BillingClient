...
set ProjFile=gbClientService.vcproj
...
set outputDir=%outputRootDir%\clientSvc_%dt%_%VersionStr%
...
xcopy /Y /D /C /Q /R %binDir%\gbClientService.exe %outputDir%
...
echo f | xcopy /Y /D /C /Q /R %commonDir%\Version\version.tag %outputDir%\version_ClientSvc.tag
...
CALL %SignToolDir%\sign_hoabinh.bat %binInstallDir%\gbClientService.exe
CALL %SignToolDir%\sign_hoabinh.bat %binInstallDir%\sqldrivers\sqlitecipher4.dll

rem also backup the pdb 
xcopy /Y /D /C /Q /R %binDir%\gbClientService.pdb %outputDir%
...
COLOR 0C
echo -----------------------------ERROR -----------------------------
echo  Build Client Service failed, read "CZUniqueBuildLogs.logcz" for details
echo -----------------------------ERROR -----------------------------
rem goto end
popd
endlocal
